...
void Allocate_arrays(double** local_A_pp, double** local_x_pp, 
      double** local_y_pp, int local_m, int n, int local_n, double** x,
      MPI_Comm comm);
...
void Mat_vect_mult(double local_A[], double local_x[], 
      double local_y[], int local_m, int n, int local_n, double x[],
      MPI_Comm comm);
...
int main(void) {
   ...
   Allocate_arrays(&local_A, &local_x, &local_y, local_m, m, local_n, &x, comm);
   Read_matrix("A", local_A, m, local_m, n, my_rank, comm);
//#  ifdef DEBUG

   Read_vector("x", local_x, n, local_n, my_rank, comm);
   
   Mat_vect_mult(local_A, local_x, local_y, local_m, m, local_n, x, comm);
...
void Read_vector(
   ...
   if (my_rank == 0) {
      ...
      //printf("The aleatory vector is: %s\n", prompt);
      for (i = 0; i < n; i++){
...
void Mat_vect_mult(
   ...
   */
   ...
   MPI_Reduce(local_y, x, m, MPI_DOUBLE, MPI_SUM, 0, comm);
